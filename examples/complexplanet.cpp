...
// off every 'zig'.)
//
// Updated for C++17 compatibility and libnoise-modern by TEK Nemesis and Grok on April 25, 2025.
// - Replaced QUALITY_STD with noise::NoiseQuality::QUALITY_STD to fix undeclared identifier errors.
// - Replaced QUALITY_BEST with noise::NoiseQuality::QUALITY_BEST to fix undeclared identifier errors.
// - Optimized performance by reducing the number of octaves in key noise modules to decrease computation time,
//   specifically targeting baseContinentDef_pe0, baseContinentDef_pe1, mountainBaseDef_rm0, mountainousHigh_rm0,
//   mountainousHigh_rm1, mountainousLow_rm0, mountainousLow_rm1, hillyTerrain_bi, plainsTerrain_bi0,
//   plainsTerrain_bi1, badlandsCliffs_pe, scaledHillyTerrain_pe, and continentalShelf_rm. This reduces detail
//   slightly but improves generation speed.
// - Adjusted the bump height in RendererNormalMap from 1.0/resInMeters (~9.26e-5) to 0.005 to
//   fix the normal map (terrainnormal.bmp) appearing mostly purple due to insufficient elevation variation, ensuring
//   proper normal vector computation with more varied RGB colors.
// - Added a console message at the start of main() to inform the user that the program has started
//   and may take a while on slower computers, improving user experience during long computations.
...
int main ()
{
    ...
    std::cout << "Starting complexplanet terrain generation...\n"
  ...
  baseContinentDef_pe0.SetOctaveCount (10); // Reduced from 14 to 10 for performance
  ...
  baseContinentDef_pe1.SetOctaveCount (8); // Reduced from 11 to 8 for performance
  ...
  mountainBaseDef_rm0.SetOctaveCount (3); // Reduced from 4 to 3 for performance
  ...
  mountainousHigh_rm0.SetOctaveCount (2); // Reduced from 3 to 2 for performance
  ...
  mountainousHigh_rm1.SetOctaveCount (2); // Reduced from 3 to 2 for performance
  ...
  mountainousLow_rm0.SetOctaveCount (6); // Reduced from 8 to 6 for performance
  ...
  mountainousLow_rm1.SetOctaveCount (6); // Reduced from 8 to 6 for performance
  ...
  hillyTerrain_bi.SetOctaveCount (4); // Reduced from 6 to 4 for performance
  ...
  plainsTerrain_bi0.SetOctaveCount (6); // Reduced from 8 to 6 for performance
  ...
  plainsTerrain_bi1.SetOctaveCount (6); // Reduced from 8 to 6 for performance
  ...
  badlandsCliffs_pe.SetOctaveCount (4); // Reduced from 6 to 4 for performance
  ...
  scaledHillyTerrain_pe.SetOctaveCount (4); // Reduced from 6 to 4 for performance
  ...
  continentalShelf_rm.SetOctaveCount (12); // Reduced from 16 to 12 for performance
  ...
  normalMapRenderer.SetBumpHeight (0.005); // Increased from 1.0 / resInMeters (~9.26e-5) to 0.005 for better normal variation
